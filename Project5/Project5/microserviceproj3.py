'''
Project: Sharding and load balancing (Project 3)
Authors: Vivek Reddy Gaddam, Soani Mustafi, Rohan Patwardhan, Atharva Pathak
Edits: Vivek Reddy Gaddam
'''
...
con5 = sqlite3.connect('user1.db', check_same_thread=False, detect_types=sqlite3.PARSE_DECLTYPES)
cur5 = con5.cursor()
# After sharding
# Posting a win or loss for a particular game, along with a timestamp and number of guesses

#making changes to this service to post the records based on uuid instead of username that was on original version
@app.post("/gamestatus/{uuid}/{gameid}/{guesses}/{won}",status_code=status.HTTP_201_CREATED)
def game_status(uuid:str, gameid: int, guesses: int, won: bool, date: typing.Optional[datetime.date] = None):
...
#Fetching the UUIS of a user as part of Service Orchestration
@app.get("/fetch/uuID/{username}")
def new_game(username: str):
	usr = cur5.execute("SELECT uu_id FROM users WHERE username = ?",[username])
	user_UUID = usr.fetchall()
	#print(usr.fetchall())
	return {"user": user_UUID, "uuid": user_UUID}
