...
def startgame(userid: str, gameid: int):
	...
	if flag == False:
		...
		return {"message": "New", "id": userid, "gameid":gameid }
...
def updategamestatus(userid: str, gameid: int, guessword: str):
	...
	for i in range(len(usergm)):
		...
		if(usergm[i]["id"] == userid and usergm[i]["game"]["gameid"] == gameid):
			...
			if(numguesses < 6):
				...
				return {"message": "Cannot enter any more guesses", "NumGuesses":0}
		else:
			present = False
	...
	if present == False:	
			...
			return {"message":"User has not started the game","NumGuesses":0}
...
#Checking the remaining guesses for a user as part of project 5
@app.get("/get_guesses_rem/{userid}/{gameid}")
def getGuessesRem(userid:str, gameid:int):
	usergm = json.loads(conn.get("Games"))
	present = False
	for i in range(len(usergm)):
		gamestatus = conn.get("Games")
		...
		if(usergm[i]["id"] == userid and usergm[i]["game"]["gameid"] == gameid):
			present = True
			numguesses = json.loads(gamestatus)[i]["game"]["numguesses"]
			remGuess = 6 - numguesses
			return {"remGuess": remGuess,"Guesses": usergm[i]["game"]["guesses"]}
		else:
			present = False
	if present == False:
		return {"message":"User has not started the game","remGuess":0}
